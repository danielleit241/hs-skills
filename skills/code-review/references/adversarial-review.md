...
### 4. Hand Off Accepted Findings

- Critical: Block merge and report the evidence. Hand off only with explicit remediation authorization.
- Medium: Recommend remediation before merge when feasible; defer only with explicit user approval.
- Low: Track for a later, explicitly authorized follow-up if the pattern repeats.
...
**Progress tracking:** When optional review tracking is used, adversarial review follows “Review implementation” and precedes evidence reporting. It never creates a fix task.
