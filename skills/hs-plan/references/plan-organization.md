...
Read root `.hs.json` before writing artifacts. Use `artifacts.plans.directory`, `artifacts.plans.reportsDirectory`, and `artifacts.plans.archiveDirectory` as the defaults. A `Plan dir:` from injected `## Naming` context is a task-specific override.
...
- After creating plan, update active-plan state only when the current platform provides such a mechanism; the artifact path remains the configured `{plan-dir}`.
