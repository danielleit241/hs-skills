...
4. **Context reminder:** `/hs:cook --auto {absolute-plan-path}/plan.md`
...
2. Read codebase docs; if stale/missing: run `/hs:scout` to search codebase
...
7. **Context reminder:** `/hs:cook {absolute-plan-path}/plan.md`
...
7. **Context reminder:** `/hs:cook --parallel {absolute-plan-path}/plan.md`
...
**Invocation:** Run `/hs:plan red-team {plan-directory-path}`.

```
/hs:plan red-team {plan-directory-path}
```
...
| `auto`   | Run `/hs:plan validate {plan-directory-path}`                      |
| `off`    | Skip validation                                                    |

**Invocation (when prompt mode, user says yes):** Run:

```
/hs:plan validate {plan-directory-path}
```
...
| Mode     | Cook Command                         |
| -------- | ------------------------------------ |
| fast     | `/hs:cook --auto {path}/plan.md`     |
| hard     | `/hs:cook {path}/plan.md`            |
| parallel | `/hs:cook --parallel {path}/plan.md` |
| two      | `/hs:cook {path}/plan.md`            |
...
After creating, update active-plan state only when the current platform provides such a mechanism; the artifact path remains the configured `{plan-dir}`.
