...
Read output locations from the repository-root `.hs.json` before saving artifacts. Use `artifacts.plans.reportsDirectory` for standalone research reports. A task-specific `Report:` path takes precedence when supplied.
...
1. **Search Strategy**:
   - **Gemini Toggle**: Check the repository-root `.hs.json` for `skills.research.useGemini` (default: `false`). If `false` or absent, skip Gemini and use WebSearch directly.
   - **Gemini Model**: Read from the repository-root `.hs.json`: `gemini.model` (default: `gemini-3-flash-preview`)
...
2. **Deep Content Analysis**:
   - When you found a potential Github repository URL, use `hs:docs-seeker` skill to find read it.
...
- If `## Naming` section is not available, save to `.hs.json` → `artifacts.plans.reportsDirectory`.
...
1. Be saved using the `Report:` path from `## Naming`, or `.hs.json` → `artifacts.plans.reportsDirectory`, with a descriptive filename
