...
```text
/hs:xia <github-url|owner/repo|local-path> [feature-description] [--compare|--copy|--improve|--port] [--auto|--fast]
```
...
1. Pack the source with `/hs:repomix`.
...
4. Use `/hs:scout` on the local project to map architecture, similar features, and integration points.
...
- activate `/hs:sequential-thinking` to trace multi-step flows
...
Do not invoke `/hs:brainstorm` from inside `xia`; that skill can create its own planning handoff and break `xia`'s phase ownership.
...
- `--compare`: write the report to `.hs.json` → `artifacts.plans.reportsDirectory` and stop
- other modes: present the plan path and hand implementation to `/hs:cook`

Implementation handoff text:

```text
Plan ready at `<artifacts.plans.directory>/<plan-dir>/plan.md`, using the value in root `.hs.json`. To implement, run /hs:cook <plan-path>.
```
